...
def lazy_calibration(func):
    ...
    def wrapper(*args, **kwargs):
        calibration_cache = kwargs['calib_images_dir'] + '/calibration_data.pickle'
...
def calibrate_camera(calib_images_dir, verbose=False):
    ...
    images = glob.glob(path.join(calib_images_dir, '*.jpg'))
    ...
    for filename in images:
        ...
        gray = cv2.resize(gray, (int(1280 * scale), int(960 * scale)), interpolation=cv2.INTER_AREA)
...
if __name__ == '__main__':
    ...
    img = cv2.imread('test_images/realsense.jpg')
