...
static int frame_send(const unsigned char *frame, int frame_len)
{
  int result, i;
...
static int command_settings(CANUSB_SPEED speed, CANUSB_MODE mode, CANUSB_FRAME frame)
{
  int cmd_frame_len;
...
static int send_data_frame(CANUSB_FRAME frame, unsigned char id_lsb, unsigned char id_msb, const unsigned char data[], int data_length_code)
{
...
static void dump_data_frames(void)
{
  int i, frame_len;
...
static int adapter_init(const char *tty_device, int baudrate)
{
  ...
  tty_fd1 = open(tty_device, O_RDWR | O_NOCTTY | O_NONBLOCK);
  if (tty_fd1 == -1) {
    fprintf(stderr, "open(%s) failed: %s\n", tty_device, strerror(errno));
    return -1;
  }

  result = ioctl(tty_fd1, TCGETS2, &tio);
  if (result == -1) {
  ...
  result = ioctl(tty_fd1, TCSETS2, &tio);
  if (result == -1) {
    fprintf(stderr, "ioctl() failed: %s\n", strerror(errno));
    close(tty_fd1);
    return -1;
