...
class IPM(object):
    ...
    def __init__(self, camera_info, ipm_info):
        ...
        for i in range(self.uv_grid.shape[1]):
            if  (self.uv_grid[0][i] > ipm_info.left) and (self.uv_grid[0][i] < ipm_info.right) and \
                (self.uv_grid[1][i] > ipm_info.top) and (self.uv_grid[1][i] < ipm_info.bottom):
                continue
            else:
                self.uv_grid[0][i] = self.uv_grid[1][i] = 0
...
class CameraIPM:
    def __init__(self):
        ...
        rospy.init_node('camera_ipm')
        self.detected = False
        self.image_sub = rospy.Subscriber("/m2d/camera/image", Image, self.callback)
        self.ipm = None
        self.px = 0
        self.py = 0
        self.camera_info = _DictObjHolder({
            "f_x": 320,             # focal length x
            "f_y": 320,             # focal length y
            "u_x": 320,             # optical center x
            "u_y": 240,             # optical center y
            "camera_height": 990,   # camera height in `mm`
            "pitch": 112.0,             # rotation degree around x
            "yaw": 0                # rotation degree around y
        })
        self.ipm_info = _DictObjHolder({
            "input_width": 640,
            "input_height": 480,
            "out_width": 640,
            "out_height": 480,
        ...
        self.ipm = IPM(self.camera_info, self.ipm_info)
    ...
    def callback(self, data):
        ...
        # Define the chessboard corners in the real world coordinates
        ...
        caffe_img = img / 255.
        caffe_img = caffe_img[:,:,(2,1,0)]
        caffe_img = caffe.io.resize(caffe_img, (480, 640))
        if len(caffe_img.shape) == 3:
            caffe_img = np.dot(caffe_img, [0.299, 0.587, 0.114])
        out_img = self.ipm(caffe_img)
