...
#    error pybind11 supports only c++17 with Intel C++ compiler v18. Use v19 or newer for c++17.
#  endif
...
/// from __cpp_future__ import (convenient aliases from c++17/17)
...
// using the new one (c++17 defect, so generally works on newer compilers, even
// if not in C++17 mode)
...
// overload_cast requires variable templates: c++17
#if defined(PYBIND11_CPP14)
...
template <typename... Args> struct overload_cast {
    static_assert(detail::deferred_t<std::false_type, Args...>::value,
                  "pybind11::overload_cast<...> requires compiling in c++17 mode");
};
#endif // overload_cast
