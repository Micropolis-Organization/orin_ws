...
* Intel compiler was not activating c++17 support due to a broken define.
...
* Replaced usage of a c++17 only construct. `#1929
...
* ``py::details::overload_cast_impl`` is available in c++17 mode, can be used
...
* Significantly reduced module binary size (10-20%) when compiled in c++17 mode
  with GCC/Clang, or in any mode with MSVC. Function signatures are now always
  precomputed at compile time (this was previously only available in c++17 mode
...
* Fixed compilation with Clang on host GCC < 5 (old libstdc++ which isn't fully
  c++17 compliant). `#1062 <https://github.com/pybind/pybind11/pull/1062>`_.
...
* Builtin support for converting C++17 standard library types and general
  ...
  1. C++17 ``std::variant`` is supported right out of the box. c++17/14
...
* Added ``py::exec()`` as a shortcut for ``py::eval<py::eval_statements>()``
  and support for c++17 raw string literals as input. See :ref:`eval`.
...
* The implementation of ``py::init<>`` now uses c++17 brace initialization
...
* Improved support for the ``/std:c++17`` and ``/std:c++latest`` modes
  on MSVC 2017.
...
* Improved support for c++17 unicode string and character types; added
...
* New ``py::overload_cast`` casting operator to select among multiple possible
  ...
  This feature only works on c++17-capable compilers.
...
1.7 (April 30, 2016)
----------------------
* Added a new ``move`` return value policy that triggers c++17 move semantics.
...
* Optional: efficient generation of function signatures at compile time using c++17
