...
/* Check that the compiler at least claims to support c++17. It might not be sufficient
 * because the compiler may not implement it correctly, but at least we'll know.
 * On the other hand, visual studio still doesn't claim to support c++17 although it's
...
#pragma GCC diagnostic error "-Wfatal-errors"
#endif
#error This library needs at least a c++17 compliant compiler. If you use g++/clang, please enable the -std=c++17 compiler flag. (-std=c++0x on older versions.)
#endif
...
/* std::get is only constexpr in c++17, not yet in c++17
 */
